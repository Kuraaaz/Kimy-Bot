const { MessageEmbed } = require('discord.js');
const UserXP = require('../../models/userXp'); // Import du modèle MongoDB

module.exports = {
  name: 'adminxp',
  description: 'Ajoute un certain nombre d\'XP à un utilisateur mentionné.',
  async execute(message, args) {
    try {
      // Vérifier si l'utilisateur a la permission d'administrateur
      if (!message.member.permissions.has('ADMINISTRATOR')) {
        return message.reply('Vous n\'avez pas la permission d\'exécuter cette commande.');
      }

      // Vérifiez si un membre a été mentionné
      const mentionedMember = message.mentions.members.first();
      if (!mentionedMember) {
        return message.reply('Veuillez mentionner un membre pour ajouter de l\'XP.');
      }

      // Vérifiez si un montant d'XP a été spécifié
      const xpToAdd = parseInt(args[1]);
      if (isNaN(xpToAdd) || xpToAdd <= 0) {
        return message.reply('Veuillez spécifier un montant d\'XP valide à ajouter.');
      }

      // Cherche l'utilisateur dans la base de données
      const userXP = await UserXP.findOne({ userId: mentionedMember.id });
      
      if (!userXP) {
        return message.reply(`L'utilisateur mentionné n'a pas encore d'XP enregistré.`);
      }

      // Ajouter l'XP
      userXP.xp += xpToAdd;
      await userXP.save();

      const embed = new MessageEmbed()
        .setTitle('XP Ajouté')
        .setDescription(`**${xpToAdd}** XP a été ajouté à **${mentionedMember.user.username}**.`)
        .setColor('#00ff00')
        .setTimestamp();

      await message.channel.send({ embeds: [embed] });

    } catch (error) {
      console.error('Une erreur est survenue lors de l\'exécution de la commande adminxp :', error);
      message.channel.send('Une erreur est survenue lors de l\'ajout de l\'XP.');
    }
  }
};
